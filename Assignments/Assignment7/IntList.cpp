...
void IntList::insert_sorted(int value)
{
    ...
    else
    {
        ...
        for(i = head; !((previous->data <= value) && (value <= current->data)); i = i->next)
        {
            previous = i->next;
            current = i ->next->next;
        }
...
IntList::IntList(const IntList& m)
{
    head = 0;
    tail = 0;
    
    for(IntNode* current = m.head; current !=0; current = current->next)
    {
        push_back(current->data);
    }
}
IntList & IntList::operator=(const IntList& m)
{
    head = 0;
    tail = 0;
...
/*void clear() // check to see if correct
{
